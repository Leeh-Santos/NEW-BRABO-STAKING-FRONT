 ...
 * builds; there is nothing stable to override from CSS.
 * `ConnectButton.Custom` is the supported escape hatch: RainbowKit keeps the
 * connection state and the modals, we own every pixel of the trigger — the horn
 * cut, the flat cyan, the mono address.
 *
 * The modals themselves are still RainbowKit's DOM; those are themed through
 * `darkTheme(...)` in App.tsx and the `--rk-*` variables in index.css. */
          ...
          >
            {!connected ? (
              ...
              <>
                ...
                {/* The account modal carries Disconnect. */}
                <button
                  type="button"
